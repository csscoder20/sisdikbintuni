<x-filament-panels::page>
    <style>
        ...
        * {
        ...
        .vd-step.done:not(:last-child)::after,
        .vd-step.active:not(:last-child)::after {
            background: var(--ok);
        }
        ...
        .vd-step.done .vd-circle {
            background: var(--ok);
            border-color: var(--ok);
            color: #fff;
        }

        .vd-step.active .vd-circle {
            background: var(--act);
        ...
        .vd-step.active .vd-label {
            color: var(--act);
            font-weight: 700;
        }

        .vd-step.done .vd-label {
            color: var(--ok);
        }
        ...
        .btn-next {
            background: linear-gradient(135deg, var(--act), #2563eb);
            color: #fff;
            margin-left: auto;
        }
                ...
                @foreach ($labels as $n => $lbl)
                    @php
                        ...
                        if ($n === $cur) {
                            ...
                            $cls .= $statuses[$n] ?? false ? ' done' : ' invalid';
                        }
                    @endphp
                    <div class="{{ $cls }}">
                        <div class="vd-circle">
        ...
        <div class="vd-card">
            ...
            <div class="vd-cb">
                ...
                @if ($cur === 1)
                    ...
                    @else
                        <div class="alert-err">
                            <p style="font-size:.8rem;color:#b91c1c;margin:0;">Terdapat <strong>{{ count($missing) }}
                                    field</strong> yang belum diisi. Silakan lengkapi data profil sekolah terlebih
                                dahulu. <a
                                    href="{{ route('filament.' . filament()->getId() . '.pages.profil', ['tenant' => filament()->getTenant()]) }}"
                                    style="color:#b91c1c;font-weight:600;">Perbaiki Sekarang!</a></p>
                        </div>
                    @endif
                            ...
                            <tbody>
                                @foreach ($rows as $r)
                                    <tr>
                                        ...
                                        <td style="color:{{ $r['ok'] ? '#111827' : '#dc2626' }};">
                                            {{ $r['value'] ?? '—' }}</td>
                                        <td>
                                            @if ($r['ok'])
                                            <span class="bOk">✓</span>@else<span class="bErr">✕</span>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
                ...
                @if ($cur === 2)
                    @php
                        $list = $this->getSarprasList();
                        $total = $this->getSarprasCount();
                    @endphp
                    @if ($total === 0)
                        <div class="alert-err">
                            <p style="font-size:.8rem;color:#b91c1c;margin:0;">Belum ada data Sarana &amp; Prasarana.
                                Tambahkan minimal 1 data gedung/ruang untuk melanjutkan. <a
                                    href="{{ route('filament.' . filament()->getId() . '.resources.laporan-gedung.index', ['tenant' => filament()->getTenant()]) }}"
                                    style="color:#b91c1c;font-weight:600;">Perbaiki Sekarang!</a></p>
                        </div>
                    @else
                        <div class="stat">
                            ...
                            <div>
                                <div class="stat-lbl">Data Sarana & Prasarana</div>
                                <div class="stat-sub">Gedung & ruang yang tercatat</div>
                            </div>
                        </div>
                    @endif
                    <div style="flex:1;overflow:auto;">
                        @if (!$list->isEmpty())
                            <table class="tbl">
                                ...
                                <tbody>
                                    @foreach ($list as $i => $s)
                                        <tr>
                                            <td>{{ ($list->currentPage() - 1) * $list->perPage() + $loop->iteration }}
                                            </td>
                                            <td>{{ $s->nama_ruang ?? '-' }}</td>
                ...
                @if ($cur === 3)
                    ...
                    @if ($total === 0)
                        <div class="alert-err">
                            <p style="font-size:.8rem;color:#b91c1c;margin:0;">Belum ada data Mata Pelajaran. Harap
                                tambahkan data mapel terlebih dahulu. <a
                                    href="{{ route('filament.' . filament()->getId() . '.resources.mapels.index', ['tenant' => filament()->getTenant()]) }}"
                                    style="color:#b91c1c;font-weight:600;">Perbaiki Sekarang!</a></p>
                        </div>
                    @else
                        @if (!$statuses[3])
                            <div class="alert-err" style="margin-bottom:1rem;">
                                <p style="font-size:.8rem;color:#b91c1c;margin:0;"><strong>Validasi Gagal:</strong>
                                    Belum ada guru yang ditugaskan ke mata pelajaran manapun. Silakan isi sebaran jam
                                    mengajar. <a
                        ...
                        <div class="stat">
                            ...
                            <div>
                                ...
                                <div class="stat-sub">Total: {{ $total }} mapel</div>
                            </div>
                        </div>
                    @endif
                    <div style="flex:1;overflow:auto;">
                ...
                @if ($cur === 4)
                    ...
                    @if ($totalRombel === 0)
                        <div class="alert-err">
                            <p style="font-size:.8rem;color:#b91c1c;margin:0;">Belum ada data Rombel. Harap tambahkan
                                rombel terlebih dahulu. <a
                    ...
                    @else
                        <div class="stat">
                            ...
                            <div>
                                <div class="stat-lbl">Rombel Terdaftar</div>
                                <div class="stat-sub">Total siswa: {{ $totalSiswaRombel }} orang</div>
                            </div>
                            @if (!empty($emptyRombels))
                                <div style="width:2px;background:#93c5fd;margin:0 10px;align-self:stretch;"></div>
                                <div style="flex:1;">
                                    <p style="font-size:.8rem;color:#dc2626;margin:0;font-weight:500;">Terdapat
                ...
                @if ($cur === 5)
                    ...
                    @if ($total === 0)
                        <div class="alert-err">
                            <p style="font-size:.8rem;color:#b91c1c;margin:0;">Belum ada data Keuangan. Harap lengkapi
                                laporan keuangan terlebih dahulu. <a
                                    href="{{ route('filament.' . filament()->getId() . '.resources.laporan-keuangan.index', ['tenant' => filament()->getTenant()]) }}"
                                    style="color:#b91c1c;font-weight:600;">Perbaiki Sekarang!</a></p>
                        </div>
                    @else
                        <div class="stat">
                            ...
                            <div>
                                ...
                                <div class="stat-sub">Total: {{ $total }} laporan</div>
                            </div>
                        </div>
                    @endif
                    <div style="flex:1;overflow:auto;">
                            ...
                            <table class="tbl">
                                <thead>
                                    <tr>
                                        ...
                                        <th width="16%" style="text-align:center;">Debit</th>
                                        <th width="16%" style="text-align:center;">Kredit</th>
                                        <th width="16%" style="text-align:center;">Saldo</th>
                                ...
                                <tbody>
                                    @foreach ($list as $i => $k)
                                        <tr>
                                            ...
                                            <td>{{ $k->tanggal ? \Carbon\Carbon::parse($k->tanggal)->translatedFormat('d F Y') : '-' }}
                                            </td>
                                            <td>{{ $k->keterangan ?? '-' }}</td>
                                            <td style="text-align:center;">
                                                {{ $k->jenis_transaksi === 'debit' ? number_format($k->nominal ?? 0, 0, ',', '.') : '' }}
                                            </td>
                                            <td style="text-align:center;">
                                                {{ $k->jenis_transaksi === 'kredit' ? number_format($k->nominal ?? 0, 0, ',', '.') : '' }}
                                            </td>
                                            <td
                                                style="text-align:center; font-weight:700;color:{{ ($k->saldo ?? 0) < 0 ? '#dc2626' : '#15803d' }};">
                                                {{ number_format($k->saldo ?? 0, 0, ',', '.') }}</td>
                                            <td style="text-align:center;"><span class="bOk">✓</span></td>
                                        </tr>
                                    @endforeach
                                </tbody>
                ...
                @if ($cur === 6)
                    @php
                        ...
                        $total = $this->getSiswaCount();
                        $incompleteSiswa = $this->getIncompleteSiswaInfo();
                    @endphp
                    @if ($total === 0)
                        <div class="alert-err">
                            <p style="font-size:.8rem;color:#b91c1c;margin:0;">Belum ada data Nominatif Siswa. Harap
                                tambahkan data siswa terlebih dahulu. <a
                    ...
                    @else
                        <div class="stat">
                            ...
                            <div>
                                ...
                                <div class="stat-sub">{{ $total - count($incompleteSiswa) }} lengkap ·
                                    {{ count($incompleteSiswa) }} belum lengkap</div>
                            </div>
                                ...
                                <tbody>
                                    @foreach ($list as $i => $s)
                                        <tr>
                                            ...
                                            <td>{{ $s->tanggal_lahir ? \Carbon\Carbon::parse($s->tanggal_lahir)->translatedFormat('d F Y') : '-' }}
                                            </td>
                                            <td>{{ $s->jenis_kelamin ? (stripos($s->jenis_kelamin, 'laki') !== false ? 'L' : 'P') : '-' }}
                                            </td>
                                            <td>{{ $s->agama ?? '-' }}</td>
                                            <td>{{ $s->alamat ?? '-' }}</td>
                                            <td>{{ $s->desa ?? '-' }}</td>
                ...
                @if ($cur === 7)
                    @php
                        $list = $this->getGtkList();
                        $total = $this->getGtkCount();
                        $incompleteGtk = $this->getIncompleteGtkInfo();
                    @endphp
                    @if ($total === 0)
                        <div class="alert-err">
                            <p style="font-size:.8rem;color:#b91c1c;margin:0;">Belum ada data Nominatif GTK. Harap
                                tambahkan data GTK terlebih dahulu. <a
                    ...
                    @else
                        <div class="stat">
                            <div class="stat-num">{{ $total }}</div>
                            <div>
                                <div class="stat-lbl">Total GTK</div>
                                <div class="stat-sub">{{ $total - count($incompleteGtk) }} lengkap ·
                                    {{ count($incompleteGtk) }} belum lengkap</div>
                            </div>
                                ...
                                <tbody>
                                    @foreach ($list as $i => $g)
                                        <tr>
                                            ...
                                            <td>{{ $g->tanggal_lahir ? \Carbon\Carbon::parse($g->tanggal_lahir)->translatedFormat('d F Y') : '-' }}
                                            </td>
                                            <td>{{ $g->jenis_kelamin ? (stripos($g->jenis_kelamin, 'laki') !== false ? 'L' : 'P') : '-' }}
                                            </td>
                                            <td>{{ $g->agama ?? '-' }}</td>
                                            <td>{{ $g->alamat ?? '-' }}</td>
                                            <td>{{ $g->desa ?? '-' }}</td>
                                            ...
                                            <td>{{ $g->tmt_pns ? \Carbon\Carbon::parse($g->tmt_pns)->translatedFormat('d F Y') : '-' }}
                                            </td>
                                            <td>{{ $g->pangkat_gol_terakhir ?? '-' }}</td>
                                            <td>{{ $g->tmt_pangkat_gol_terakhir ? \Carbon\Carbon::parse($g->tmt_pangkat_gol_terakhir)->translatedFormat('d F Y') : '-' }}
                                            </td>
                                            <td style="text-align:center;">
                                                @if ($this->isGtkComplete($g))
                                                <span class="bOk">✓</span>@else<span class="bErr">✕</span>
                                                @endif
                                            </td>
                                        </tr>
                ...
                @if ($cur === 8)
                    ...
                    @else
                        <div class="stat">
                            <div class="stat-num">{{ $total }}</div>
                            <div>
                                <div class="stat-lbl">Riwayat Pendidikan GTK</div>
                                <div class="stat-sub">{{ $total - count($tanpaPendidikan) }} lengkap ·
                                    {{ count($tanpaPendidikan) }} belum mengisi</div>
                            </div>
                                ...
                                <tbody>
                                    @foreach ($list as $g)
                                        ...
                                        <tr>
                                            ...
                                            {{-- D1 --}}
                                            <td>{{ $pend->thn_tamat_d1 ?? '-' }}</td>
                                            <td>{{ $pend->jurusan_d1 ?? '-' }}</td>
                                            <td>{{ $pend->perguruan_tinggi_d1 ?? '-' }}</td>
                                            {{-- D2 --}}
                                            <td>{{ $pend->thn_tamat_d2 ?? '-' }}</td>
                                            <td>{{ $pend->jurusan_d2 ?? '-' }}</td>
                                            <td>{{ $pend->perguruan_tinggi_d2 ?? '-' }}</td>
                                            {{-- D3 --}}
                                            <td>{{ $pend->thn_tamat_d3 ?? '-' }}</td>
                                            <td>{{ $pend->jurusan_d3 ?? '-' }}</td>
                                            <td>{{ $pend->perguruan_tinggi_d3 ?? '-' }}</td>
                                            {{-- S1 --}}
                                            <td>{{ $pend->thn_tamat_s1 ?? '-' }}</td>
                                            <td>{{ $pend->jurusan_s1 ?? '-' }}</td>
                                            <td>{{ $pend->perguruan_tinggi_s1 ?? '-' }}</td>
                                            {{-- S2 --}}
                                            <td>{{ $pend->thn_tamat_s2 ?? '-' }}</td>
                                            <td>{{ $pend->jurusan_s2 ?? '-' }}</td>
                                            <td>{{ $pend->perguruan_tinggi_s2 ?? '-' }}</td>
                                            {{-- S3 --}}
                                            <td>{{ $pend->thn_tamat_s3 ?? '-' }}</td>
                                            <td>{{ $pend->jurusan_s3 ?? '-' }}</td>
                                            <td>{{ $pend->perguruan_tinggi_s3 ?? '-' }}</td>
                                            {{-- Akta --}}
                                            <td>{{ $pend->thn_tamat_akta4 ?? '-' }}</td>
                                            <td>{{ $pend->jurusan_akta4 ?? '-' }}</td>
                                            <td>{{ $pend->perguruan_tinggi_akta4 ?? '-' }}</td>

                                            {{-- Gelar --}}
                                            <td>{{ $pend->gelar_depan ?? '-' }}</td>
                                            <td>{{ $pend->gelar_belakang ?? '-' }}</td>

                                            <td style="text-align:center;">
                                                @if (in_array($g->nama, $tanpaPendidikan))
                                                <span class="bErr">✕</span>@else<span class="bOk">✓</span>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
                ...
                @if ($cur === 9)
                    ...
                    @else
                        <div class="stat">
                            <div class="stat-num">{{ $total }}</div>
                            <div>
                                <div class="stat-lbl">Rekening & NPWP GTK</div>
                                <div class="stat-sub">{{ $total - count($tanpaRekening) }} lengkap ·
                                    {{ count($tanpaRekening) }} belum mengisi</div>
                            </div>
                                ...
                                <tbody>
                                    @foreach ($list as $g)
                                        <tr>
                                            ...
                                            <td>{{ $g->npwp ?? '-' }}</td>
                                            <td style="text-align:center;">
                                                @if (in_array($g->nama, $tanpaRekening))
                                                <span class="bErr">✕</span>@else<span class="bOk">✓</span>
                                                @endif
                                            </td>
                                        </tr>
                ...
                @if ($cur === 10)
                    ...
                    @if ($total === 0)
                        <div class="alert-err">
                            <p style="font-size:.8rem;color:#b91c1c;margin:0;">Belum ada data Sebaran Jam Mengajar.
                                Harap isi data jam mengajar terlebih dahulu. <a
                                    href="{{ route('filament.' . filament()->getId() . '.resources.sebaran-jam-ajar.index', ['tenant' => filament()->getTenant()]) }}"
                                    style="color:#b91c1c;font-weight:600;">Perbaiki Sekarang!</a></p>
                        </div>
                    @else
                        <div class="stat">
                            <div class="stat-num">{{ $total }}</div>
                            <div>
                                <div class="stat-lbl">Total GTK yang Mengajar</div>
                                <div class="stat-sub">Total: {{ $total }} GTK</div>
                            </div>
                            ...
                            <table class="tbl">
                                <thead>
                                    <tr>
                                        <th width="5%">No</th>
                                        <th>Nama GTK</th>
                                        <th width="15%">Jenis GTK</th>
                                        <th width="12%">Jml Entri</th>
                ...
                @if ($cur === 11)
                    ...
                    @if ($total === 0)
                        <div class="alert-err">
                            <p style="font-size:.8rem;color:#b91c1c;margin:0;">Belum ada data Kehadiran GTK. Harap isi
                                rekap kehadiran terlebih dahulu. <a
                    ...
                    @else
                        <div class="stat">
                            <div class="stat-num">{{ $total }}</div>
                            <div>
                                <div class="stat-lbl">Rekap Kehadiran GTK</div>
                                <div class="stat-sub">
                                    {{ count($gtkTanpaKehadiran) > 0 ? count($gtkTanpaKehadiran) . ' GTK belum tercatat' : 'Semua GTK sudah tercatat' }}
                                </div>
                            </div>
                ...
                @if ($cur === 12)
                    <div
                        style="flex:1; padding: 3rem 2rem; text-align: center; background: #fafafa; border-radius: 8px; border: 1px dashed #cbd5e1;">
                        <div style="font-size: 3rem; margin-bottom: 1rem;">📄</div>
                        <h3 style="color: #1e293b; font-size: 1.25rem; font-weight: 700; margin-bottom: 0.5rem;">
                            Pernyataan Validasi</h3>

                        @if ($allValid)
                            <p style="color: #15803d; font-weight: 600; margin-bottom: 1.5rem;">🎉 Semua langkah valid!
                                Siap untuk disimpan.</p>
                        @endif

                        <p
                            style="font-size: 1.1rem; color: #333; line-height: 1.6; font-weight: 500; margin-bottom: 2rem;">
                            "Data yang saya masukkan sudah benar dan data akan direkam sebagai laporan resmi<br>periode
                            bulan {{ \Carbon\Carbon::now()->translatedFormat('F') }} tahun
                            {{ \Carbon\Carbon::now()->translatedFormat('Y') }}."
                        </p>

                        @if ($locked)
                            <p style="font-size: 0.85rem; color: #991b1b; font-weight: 600;">Periode validasi sedang
                                ditutup oleh Admin Dinas.</p>
                        @else
                            <p style="font-size: 0.85rem; color: #64748b;">Klik tombol <b>Selesai & Simpan Validasi</b>
                                di bawah jika Anda setuju.</p>
                        @endif
                    </div>
                @endif
            ...
            @unless ($locked)
                {{-- Footer actions --}}
                <div class="vd-footer">
                    <button wire:click="prevStep" class="btn btn-back" {{ $cur <= 1 ? 'disabled' : '' }}>←
                        Kembali</button>
