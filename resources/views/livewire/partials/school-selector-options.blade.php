<div style="max-height: 250px; overflow-y: auto; display: flex; flex-direction: column; gap: 2px;">
    ...
    @foreach ($sekolahs as $sekolah)
        <div wire:click="selectSekolah({{ $sekolah->id }})" @click="{{ $closeExpression }}"
            style="padding: 8px 12px; font-size: 0.875rem; cursor: pointer; border-radius: 6px; color: #111827; transition: all 0.2s; text-transform: uppercase; {{ $selectedSekolahId == $sekolah->id ? 'background-color: #ffedd5; color: #9f9fa9; font-weight: 600;' : '' }}"
